...
app.get('/posts/:topic', function(req,res){
  ...
  myArray.forEach(function(item){
    const reqitem= _.lowerCase(item.title);
    if(reqparam===reqitem){
      res.render('post', {title:item.title, content:item.content});
    }
  });

  // if (req.params.topic===req.body.blogtitle){
...
app.post('/compose',function(req,res){
  ...
  myArray.push(newblog);
  // var obj=JSON.stringify(req.body);  This can also be used to output the data in json object format
  // console.log(obj);
  res.redirect('/');
